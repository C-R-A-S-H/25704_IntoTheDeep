...
import org.firstinspires.ftc.teamcode.utility_code.WallFollowAssist;

@TeleOp(name = "DinoTeleOp_State")
public class DinoTeleOp_State extends LinearOpMode {

    private DcMotor BR_Motor, BL_Motor, FL_Motor, FR_Motor;
    private DcMotorEx IL_L, IL_R, Arm_L, Arm_R;
    ...
    private static int ARM_L_PICKUP_TARGET_POSITION = 1200;    // Adjust this value based on testing
    private static int ARM_R_PICKUP_TARGET_POSITION = 1200;    // Adjust this value based on testing
    ...
    private static int ARM_L_CLIPPING_TARGET_POSITION = 2940;    // Adjust this value based on testing
    private static int ARM_R_CLIPPING_TARGET_POSITION = 2940;    // Adjust this value based on testing
    ...
    private int arm_LMinPosition = Integer.MAX_VALUE;     //This sets up the lowest value of the Arm motor encoder
    private int arm_RMinPosition = Integer.MAX_VALUE;     //This sets up the lowest value of the Arm motor encoder
    ...
    public void runOpMode() {
        ...
        Arm_L = hardwareMap.get(DcMotorEx.class, "Arm_Left");
        Arm_R = hardwareMap.get(DcMotorEx.class, "Arm_Right");
...
//        Arm_L.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
...
//        IL_R.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
        Arm_L.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
        Arm_R.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
...
//                int currentArm_LPos = Arm_L.getCurrentPosition();
//                int currentArm_RPos = Arm_R.getCurrentPosition();
//                int currentIL_LPos = IL_L.getCurrentPosition();
//                int currentIL_RPos = IL_R.getCurrentPosition();
/*
                // Update minimum positions
                if (currentArm_LPos < arm_LMinPosition) {
                    arm_LMinPosition = currentArm_LPos;
                    ARM_L_PICKUP_TARGET_POSITION = arm_LMinPosition + 1265;
                }

                if (currentArm_LPos < arm_LMinPosition) {
                    arm_LMinPosition = currentArm_LPos;
                    ARM_L_CLIPPING_TARGET_POSITION = arm_LMinPosition + 2940;
                }
                // Update minimum positions
                if (currentArm_RPos < arm_RMinPosition) {
                    arm_RMinPosition = currentArm_RPos;
                    ARM_R_PICKUP_TARGET_POSITION = arm_RMinPosition + 1265;
                }

                if (currentArm_RPos < arm_RMinPosition) {
                    arm_RMinPosition = currentArm_RPos;
                    ARM_R_CLIPPING_TARGET_POSITION = arm_RMinPosition + 2940;
                }
*/
/*
...
*/
                ...
                if (gamepad1.x) {
                    ...
                    ((DcMotorImplEx) Arm_L).setVelocity(-800);
                    ((DcMotorImplEx) Arm_R).setVelocity(-800);
                }
...
//                    boolean arm_LMovingUp = ARM_L_PICKUP_TARGET_POSITION > currentArm_LPos;
//                    boolean arm_RMovingUp = ARM_R_PICKUP_TARGET_POSITION > currentArm_RPos;
...
/*
                    if (arm_LMovingUp) {
                        Arm_L.setTargetPosition(ARM_L_PICKUP_TARGET_POSITION + OVERSHOOT_UP);
                    } else {
                        Arm_L.setTargetPosition(ARM__LPICKUP_TARGET_POSITION - UNDERSHOOT_DOWN);
                    }
                    if (arm_RMovingUp) {
                        Arm_R.setTargetPosition(ARM_R_PICKUP_TARGET_POSITION + OVERSHOOT_UP);
                    } else {
                        Arm_R.setTargetPosition(ARM_R_PICKUP_TARGET_POSITION - UNDERSHOOT_DOWN);
                    }

                    Arm_L.setMode(DcMotor.RunMode.RUN_TO_POSITION);
                    Arm_R.setMode(DcMotor.RunMode.RUN_TO_POSITION);
                    Arm_L.setPower(MOVE_POWER);
                    Arm_R.setPower(MOVE_POWER);
*/
/*
                    if (il_LMovingUp) {
...
*/
//                    telemetry.addData("Arm Direction", arm_LMovingUp ? "UP" : "DOWN");
...
//                    boolean arm_LMovingUp = ARM_L_CLIPPING_TARGET_POSITION > currentArm_LPos;
//                    boolean arm_RMovingUp = ARM_R_CLIPPING_TARGET_POSITION > currentArm_RPos;
...
/*
                    if (arm_LMovingUp) {
                        Arm_L.setTargetPosition(ARM_L_CLIPPING_TARGET_POSITION + OVERSHOOT_UP);
                    } else {
                        Arm_L.setTargetPosition(ARM_L_CLIPPING_TARGET_POSITION - UNDERSHOOT_DOWN);
                    }
                    if (arm_RMovingUp) {
                        Arm_R.setTargetPosition(ARM_R_CLIPPING_TARGET_POSITION + OVERSHOOT_UP);
                    } else {
                        Arm_R.setTargetPosition(ARM_R_CLIPPING_TARGET_POSITION - UNDERSHOOT_DOWN);
                    }
 ...
 */
 /*
                    ...
                    IL_R.setPower(MOVE_POWER);
  */
//                    telemetry.addData("Arm_L Direction", arm_LMovingUp ? "UP" : "DOWN");
...
//                    int arm_LTarget = Arm_L.getTargetPosition();
//                    int arm_RTarget = Arm_R.getTargetPosition();
//                    int il_LTarget = IL_L.getTargetPosition();
//                    int il_RTarget = IL_R.getTargetPosition();

                    // Check if we're within tolerance of target
//                    boolean arm_LAtTarget = Math.abs(currentArm_LPos - arm_LTarget) < POSITION_TOLERANCE;
//                    boolean arm_RAtTarget = Math.abs(currentArm_RPos - arm_RTarget) < POSITION_TOLERANCE;
...
//                    double armVelocity = Math.abs(Arm_L.getVelocity());
//                    double armVelocity = Math.abs(Arm_R.getVelocity());
...
//                    boolean arm_LStalled = arm_LVelocity < 10 && Math.abs(Arm_L.getPower()) > 0.1;
//                    boolean arm_RStalled = arm_RVelocity < 10 && Math.abs(Arm_R.getPower()) > 0.1;
...
//                    boolean il_RStalled = il_RVelocity < 10 && Math.abs(IL_R.getPower()) > 0.1;
                    ...
                    if ((!Arm_L.isBusy()) || // Original condition
                        (arm_LAtTarget )||    // New: position reached within tolerance
                        timeout ||                        // New: timeout reached
                        (arm_LStalled )) {     // New: motors stalled
                    if ((!Arm_R.isBusy()) || // Original condition
                            (arm_RAtTarget )||    // New: position reached within tolerance
                            timeout ||                        // New: timeout reached
                            (arm_RStalled )) {     // New: motors stalled
...
//                        IL_R.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
                        ...
                        telemetry.addData("Exit Reason",
                            timeout ? "Timeout" :
//                            arm_LStalled ? "Motors Stalled" :
//                            arm_LAtTarget ? "Position Reached" :
...
//                        telemetry.addData("Arm Error", Math.abs(currentArm_LPos - arm_LTarget));
...
//                    telemetry.addData("Arm_L Position", currentArm_LPos);
...
*/                }
                ...
                if (Math.abs(armPower) > STICK_DEADZONE || Math.abs(liftPower) > STICK_DEADZONE) {
                    ...
                    //                   if (Arm_L.getMode() != DcMotor.RunMode.RUN_USING_ENCODER) {
                    //                       Arm_L.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
                    //                   if (Arm_R.getMode() != DcMotor.RunMode.RUN_USING_ENCODER) {
                    //                       Arm_R.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
...
/*
  ...
  */
                    if (Math.abs(armPower) > STICK_DEADZONE) {
                        Arm_L.setPower(armPower * motorARMSpeed);
                        Arm_R.setPower(armPower * motorARMSpeed);
                    }
                    if (Math.abs(liftPower) > STICK_DEADZONE) {
...
//                telemetry.addData("Arm_L Min Position", arm_LMinPosition);
//                telemetry.addData("IL_L Min Position", il_LMinPosition);
//                telemetry.addData("Arm_L Current Position", currentArm_LPos);
